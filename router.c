...
ROUTE_GET("/eran"){
 ...
 printf("HTTP/1.1 200 OK\r\n\r\n");
    ...
    ROUTE_POST("/pass")
    {
        ...
        // gets(supplied_password);
        // if(strcmp(supplied_password,"Aa1234") == 0)
        // {
        //     auth = true;
        // }
        // if(auth)
    ...
    ROUTE_POST("/register")
    {
        // פותחים את הקובץ במצב append
        int *fp = fopen("password.txt", "a");
        if (fp == NULL) {
            perror("Error opening file");
            return EXIT_FAILURE;
        }

        // כותבים שורה חדשה לסוף הקובץ
        fprintf(fp, "זוהי שורה חדשה שנוספה לסוף הקובץ\n");
